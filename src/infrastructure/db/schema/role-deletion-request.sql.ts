...
export const roleDeletionRequests = mysqlTable(
  ...
  (table) => ({
    roleIdIndex: index("role_deletion_requests_role_id_idx").on(table.roleId),
    statusRequestedAtIndex: index(
      "role_deletion_requests_status_requested_at_idx",
